...
#   1. ln5_ref  3-robot, night3 recipe + SWING MATCH 0.5   (swing on 3 robots)
#   2. ln5_tok  same + FSQ token from the RETRAINED v2 tokenizer (250 epochs,
...
#   > experiments/local_3t/ln5_run.log 2>&1 < /dev/null &
set -u
...
echo "[ln5] waiting for tokenizer_3t_v2 sidecars..."
for i in $(seq 1 240); do
  [ -f "$TOKV2/BoosterT1/dance2_subject4_zq.npz" ] && break
  sleep 60
done
[ -f "$TOKV2/BoosterT1/dance2_subject4_zq.npz" ] || { echo "[ln5] tokenizer never finished"; exit 1; }
...
cmp -s "$TOKV2/BoosterT1/dance2_subject4.npz" "$ORIG/BoosterT1/dance2_subject4.npz" \
  && { echo "[ln5] ABORT: v2 T1 clip is the INFEASIBLE original"; exit 1; }
echo "[ln5] v2 sidecars present and T1 clip is the fixed one"
...
run_arm() {  # run_arm <name> <clipdir> <latent True|False>
  ...
  while [ "$attempt" -lt 6 ]; do
    ...
    echo "[ln5] START $NAME attempt=$attempt latent=$LOBS resume=${ckpt:-none}  $(date -Is)"
    date +%s > "$OUT/${NAME}.heartbeat"
    $PY experiment.py "${WORLD[@]}" ${RESUME[@]+"${RESUME[@]}"} >> "$log" 2>&1
    local rc=$?
    local steps; steps=$(steps_done "$log"); steps=${steps:-0}
    echo "[ln5] END   $NAME attempt=$attempt rc=$rc steps=$steps  $(date -Is)"
    if [ "$steps" -ge "$TOTAL" ] 2>/dev/null; then
      touch "$OUT/${NAME}.done"; echo "[ln5] OK $NAME"; return 0
    fi
    if [ "$steps" -le "$last_steps" ]; then
      echo "[ln5] NO PROGRESS -- giving up on $NAME"; return 1
    fi
    last_steps=$steps
    sleep 20
  done
...
run_arm ln5_ref "$MERGED" False
run_arm ln5_tok "$TOKV2" True

echo "[ln5] === eval phase ==="
EOUT=$OUT/ln5_eval; RND=$OUT/renders_ln5
mkdir -p "$EOUT" "$RND"
cd "$REPO/experiments/fsq_khaendler"
MR=$(newest_ckpt ln5_ref); MT=$(newest_ckpt ln5_tok)
echo "[ln5] models: $MR | $MT"
...
evalrun() {  # <tag> <model> <cdir> <zero> <seed> <lat> [dump]
  ...
  echo "[ln5eval] $1 s$5 $(date -Is)"
  $PY "$REPO/experiments/fsq_khaendler/crosseval_motion.py" "${args[@]}" > "$EOUT/${1}_s${5}.log" 2>&1
  [ -s "$EOUT/${1}_s${5}.json" ] && echo "  OK" || echo "  MISSING"
}
evalrun ln5_ref_policy "$MR" "$MERGED" 0 0 0 ln5_ref
evalrun ln5_tok_policy "$MT" "$TOKV2"  0 0 1 ln5_tok
evalrun ln5_zero       "$MR" "$MERGED" 1 0 0
for s in 1 2 3; do
  evalrun ln5_ref_policy "$MR" "$MERGED" 0 "$s" 0
  evalrun ln5_tok_policy "$MT" "$TOKV2"  0 "$s" 1
  evalrun ln5_zero       "$MR" "$MERGED" 1 "$s" 0
done
echo "[ln5] ALL DONE $(date -Is)"
