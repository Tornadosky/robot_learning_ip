...
def make_eval_checkpoint(model_path: str, work_dir: str) -> str:
    """Repack one checkpoint and remove resume-only state; weights are unchanged."""
    import zipfile

    src = Path(model_path)
    unpacked = Path(work_dir) / "unpacked"
    shutil.unpack_archive(str(src), str(unpacked), "zip")
    removed = []
    for name in ("training_progress.json", "resume_manifest.json", "resume_state.npz"):
        candidate = unpacked / name
        if candidate.exists():
            candidate.unlink()
            removed.append(name)
    dst = Path(work_dir) / "eval.model"
    with zipfile.ZipFile(dst, "w", zipfile.ZIP_DEFLATED) as archive:
        for path in sorted(unpacked.rglob("*")):
            if path.is_file():
                archive.write(path, path.relative_to(unpacked).as_posix())
    print(f"evaluation checkpoint: {dst} (stripped: {', '.join(removed) or 'none'})", flush=True)
    return str(dst)
...
def main() -> None:
    ...
    p.add_argument("--observe_heading", default="True", choices=("True", "False"),
                   help="tracking_clip_observe_root_heading, as trained")
    p.add_argument("--command_cap_scale", type=float, default=8.0,
                   help="tracking_clip_command_cap_scale, as trained")
    p.add_argument("--morphology_coeff", type=float, default=0.0,
                   help="fixed seen-robot morphology magnitude for this rollout")
    p.add_argument("--exact_inertia_rescale", default="True", choices=("True", "False"))
    p.add_argument("--torque_scaling_exponent", type=float, default=4.0)
    p.add_argument("--joint_temperature", type=float, default=0.05)
    p.add_argument("--heading_kernel", default="cosine", choices=("cosine", "exponential"))
    ...
    config.environment.reward.joint_tracking_temperature = args.joint_temperature
    ...
    config.environment.command.tracking_clip_observe_root_heading = (args.observe_heading == "True")
    config.environment.command.tracking_clip_velocity_command = True
    config.environment.command.tracking_clip_command_cap_scale = args.command_cap_scale
    config.environment.command.tracking_reference_action_bias = args.refbias
    config.environment.reward.root_heading_tracking_kernel = args.heading_kernel
    ...
    # Isolate seen-body morphology during the randomized evaluation: no sensor
    # noise, pushes, MuJoCo-global randomization, unseen factors, delay, or dropout.
    config.environment.domain_randomization.seen_robot.morphology_coeff_mode = "fixed"
    config.environment.domain_randomization.seen_robot.morphology_coeff_value = args.morphology_coeff
    config.environment.domain_randomization.seen_robot.exact_inertia_rescale = (args.exact_inertia_rescale == "True")
    config.environment.domain_randomization.seen_robot.torque_scaling_exponent = args.torque_scaling_exponent
    config.environment.domain_randomization.sampling_type = (
        "step_probability_and_reset" if args.morphology_coeff > 0.0 else "none"
    )
    config.environment.domain_randomization.perturbation.sampling_type = "none"
    config.environment.domain_randomization.perturbation.type = "none"
    config.environment.domain_randomization.observation_noise.type = "none"
    config.environment.domain_randomization.mujoco_model.type = "none"
    config.environment.domain_randomization.unseen_robot.type = "none"
    config.environment.domain_randomization.action_delay.type = "none"
    config.environment.domain_randomization.joint_dropout.type = "none"
            ...
            if n_ok:
                for k, v in state.info.items():
                    ...
                    if vals.shape[0] != ok.shape[0]:
                        continue
                    if k.startswith("env_info/foot") or k.startswith("env_info/ref_foot"):
                        d["foot"][k] = d["foot"].get(k, 0.0) + float(vals[ok].sum())
                    if k.startswith("env_info/") and any(token in k for token in (
                        "joint_tracking_error", "qvel_tracking_error", "rpos_tracking_error",
                        "rquat_tracking_error", "root_heading_error", "foot_height_error",
                        "foot_penetration_m", "foot_clearance_m", "foot_slip_speed_sq",
                    )):
                        d["env_metrics"][k] = d["env_metrics"].get(k, 0.0) + float(vals[ok].sum())
                d["foot_n"] += n_ok
                d["env_metric_n"] += n_ok
    ...
    result = {"model_path": args.model_path, "clip_dir": args.clip_dir,
              ...
              "eval_condition": {
                  ...
                  "robots": list(robots), "seed": args.seed, "refbias": args.refbias,
                  "anchor": args.anchor, "fitvariant": args.fitvariant,
                  "refroot": args.refroot, "refroot_floor": args.refroot_floor,
                  "refvel_obs": args.refvel_obs, "observe_heading": args.observe_heading,
                  "command_cap_scale": args.command_cap_scale,
                  "morphology_coeff": args.morphology_coeff,
                  "exact_inertia_rescale": args.exact_inertia_rescale,
                  "torque_scaling_exponent": args.torque_scaling_exponent,
                  "joint_temperature": args.joint_temperature,
                  "heading_kernel": args.heading_kernel,
    ...
    for d in per:
        ...
        result["robots"][d["robot"]] = {
            ...
            **_heading_stats(d),
            ...
            "environment_tracking_metrics": {
                k.split("/")[1]: v / max(1, d["env_metric_n"])
                for k, v in sorted(d["env_metrics"].items())
            },
        }
