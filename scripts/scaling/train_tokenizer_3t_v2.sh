#!/usr/bin/env bash
# tokenizer_3t_v2 -- the PROPER 3-robot tokenizer (night5).
# Fixes found in the 2026-08-31 debug session:
#   * v1 was trained 40 epochs and reconstructs at 0.35-0.37 rad (in-corpus AND
#     held-out) vs the production mc tokenizer's 0.04 at 250 epochs. The ln3_tok
#     arm consumed that weak code. -> 250 epochs.
#   * --foot-channels: the two-head/A5 variant (decoder must reconstruct both
#     feet's heights) so the code carries swing/contact info. z_q layout
#     unchanged -> drop-in for the RL side.
#   * walk1 held out entirely -> its emitted tokens are the honest zero-shot
#     probe for RL arms trained on this tokenizer.
# Then emit design-B sidecars for dance2_subject4 + walk1, all 3 robots.
set -eu
cd "$(dirname "$0")/../.."
...
echo "=== emit v2 sidecars (design B: original npz + z_q) ==="
